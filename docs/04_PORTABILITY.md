# BacForge — Taşınabilirlik (Portability) Sözleşmesi
...
|----------|--------|-----------|
| `BACFORGE_HOME` | Projenin kök dizini | Yeni yolu göster, yeter |
| `BACFORGE_DB`   | Büyük veritabanlarının dizini | DB'yi ayrı taşı, yolu güncelle |
| `BACFORGE_WORK` | Çıktı/çalışma dizini | İstersen ayrı diske al |
...
- `BACFORGE_DB` altında, **sürüm etiketli** klasörlerde durur (`bakta/db-v6.0/`, `gtdbtk/release220/`…).
- Proje taşınınca DB'yi yeniden indirmeye gerek yok; sadece `BACFORGE_DB` yolunu göster.
...
- Çıktı: `BACFORGE_WORK/<run_id>/00_Project … 19_Logs`.
...
- Proje içi referanslar göreli (`$BACFORGE_HOME/modules/...`).
- `MANIFEST.sha256`: taşıma sonrası bütünlük doğrulaması.

## 8. Web servise geçişe hazır sınır
- Pipeline çekirdeği saf bir kütüphane/CLI (`bacforge`) olur; web katmanı (FastAPI) bunu **dışarıdan** çağırır.
...
```
1. Proje klasörünü kopyala (DB'ler hariç — onlar BACFORGE_DB'de)
